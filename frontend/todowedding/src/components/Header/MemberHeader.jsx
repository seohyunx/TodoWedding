...
const MemberHeader = ({ marryDt, loginUserNickname }) => {
      ...
      .then((res) => {
        if (res.data != "") {
          ...
          setLatestSchedule({
            ...
            schedule_start_dt:
              dateData[0] + "년 " + dateData[1] + "월 " + dateData[2] + "일",
          });
        } else {
  ...
  return (
    <div
      ...
      style={{
        backgroundImage: "linear-gradient(to right, #D4C7F9, white)",
    ...
    >
      {marryDt ? (
        <Link
          to="/"
          className="decoration-transparent self-cente pl-3 pt-4 ml-4 text-[#9F7FFC] font-bold "
        >
          <div
            className="decoration-solid underline underline-offset-8 text-[25px]"
            id="marryFont"
          >
            D{marryDt}
          </div>
          ...
          {latestSchedule != "none" ? (
            <>
              <span className="font-bold">
                {latestSchedule.schedule_start_dt}
              </span>
              에 <br></br>
            ...
            <div>
              <span>
                <span className="font-bold text-[#9F7FFC]">TodoWedding</span>과
                함께 <br></br>
                쉽고 편하게 결혼을 준비해보세요!
              </span>
            </div>
      ...
      <Link
        ...
        className="border p-2 mr-1 self-center header-hamburger"
      >
